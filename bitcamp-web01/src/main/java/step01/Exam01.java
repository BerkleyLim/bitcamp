...
//    - 서블릿 컨테이너
//      예) 톰캣 서버
// => 일반적인 실행할 수 있는 자바 클래스를 만들 듯이 main() 메서드를 만들어봐야 소용없다.
// => 서블릿 컨테이너가 호출할 클래스이기 때문에,
//    서블릿 컨테이너가 호출하는 메서드를 갖는 클래스를 만들어야 한다.
// => 서블릿 컨테이너가 어떤 메서드를 호출할 지 이미 규칙으로 정의되어 있다.
//    바로 그 규칙이 Servlet 인터페이스인 것이다.

// 이렇게 만든 서블릿을 실행하려면 실행 목록에 서블릿 컨테이너에 등록해야 한다.
...
public class Exam01 implements Servlet {
    ...
    @Override
    public void destroy() {
        // 앱 어플리케이션을 멈추거나 서블릿 컨테이너를 종료할 때 호출된다.
