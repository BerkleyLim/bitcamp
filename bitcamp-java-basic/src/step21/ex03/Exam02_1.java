...
public class Exam02_1 {
    ...
    //    => JVM에서 발생된 오류이다.
    //    => 개발자가 사용하는 오류가 아니다.
    //    => 이 오류가 발생하면 현재의 시스템의 상태를 백업하고, 즉시 실행을 멈춰야 한다.
    //    => JVM에서 오류가 발생한 경우에는 계속 실행해봐야 소용이 없다.
    //       근본적으로 문제를 해결할 수 없다.
    //    => 오류의 예:
    //       스택 오버 플로우, VM 관련 오류, 윈도우 라이브러리 관련 오류, 스레드 종료 오류 등
    
    // 2) java.lang.Exception (애플리케이션 오류)
    //    => 애플리케이션에서 발생시킨 오류이다.
    //    => 개발자가 사용하는 클래스이다.
    //    => 적절한 조치를 취한 후 계속 시스템을 시행하게 만들 수 있다.
    //    => 오류의 예:
    ...
    // 예외처리 할 때에는 기본적으로 Exception을 지정해야한다.
    static void m4() throws Exception {
        throw new Exception();  // OK! 보통 걔발자가 사용하는 예외 클래스이다.
    }
