// IoC 컨테이너 만들기 - 객체를 저장할 때 클래스 이름으로 저장하기, 클래스 이름으로 객체 꺼내긴
package step19.ex07;
...
// => 그러나 현업에서는 인스턴스와 객체와 같은 의미로 사용한다.
// => bean = instance = object

// 빈 컨테이너(bean container)?
// => 위의 ApplicationContext8과 같이 인스턴스를 생성하고 보관하고 
//    필요할 때 리턴해주는 그런 역할을 하는 객체를 말한다.
// => 컨테이너라는 객체의 '생성-실행-소멸'즉 객체의 생명주기(lifecycle)를 관리한다.
// => 대표적인 라이브러리가 "스프링 IoC 컨테이너" 이다.

// IoC 컨테이너(Inersion of Control 컨테이너)?
