// 변수 - 변수 선언과 값 저장

package step02;

public class Exam03_4 {
    public static void main(String[] args) {
        // <변수 선언 방법>
        // 1) 변수를 선언한 후 값 저장
        // - 즉, 모든 메모리를 준비한 후 값 저장하기
        int a1, a2;
        a1 = 100;
        a2 = 200;

        System.out.println("a1의 값 : " + a1);
        System.out.println("a2의 값 : " + a2);

        // 2) 변수 선언과 동시에 값을 저장하기
        // - 메모리를 준비한 후 즉시 값으로 초기화시키기
        int b1 = 100, b2 = 200;

        System.out.println("b1의 값 : " + b1);
        System.out.println("b2의 값 : " + b2);

        // 물론 여러개의 변수를 나열할 때 모든 변수에 값을 초기화 할 필요 없음
        int c1 = 100, c2, c3 = 300, c4;

        System.out.println("c1의 값 : " + c1);

        // 메모리를 준비한 후 단 한번이라도 값을 넣은 후에 사용해야한다.
        // 만약 c2 or c4 처럼 값을 메모리를 어떤 값으로 초기화 시키지 않은 상태에서
        // 출력하거나 사용하려하면 컴파일 오류가 발생한다.
        //System.out.println("c2의 값 : " + c2);    // 컴파일 오류

        System.out.println("c3의 값 : " + c3);

        //System.out.println("c4의 값 : " + c4);    // 컴파일 오류

    }
}

// 