...
// - 오직 가비지 컬렉터(garbage collector)만이 가비지를 해제 가능
...
//   1) 메모리가 부족한 경우
//    - OS에게 메모리를 요청하기 전에 먼저 가비지를 청소한 다음에 요청할지 말지 결정
//   2) CPU가 한가 할 때
...
//    - 일종의 빠른 시일내에 가비지 실행을 JVM에 예약하는 것이다.
...
//   따라서 메모리를 임의로 해제할 생각을 버려라!!

// 컬리지 컬렉션(garbe collection)?
// - 가비지 컬렉터를 통해서 가비지를 해제시키는 것.

// 가비지 컬렉션 방식의 문제점?
// - 가비지 컬렉터가 실행되기 전까지는 일정 시간 쓰레기 메모리가
//   유지되게 된다.
// - 메모리를 사용률을 극한으로 끌어 올릴 수 없다.
