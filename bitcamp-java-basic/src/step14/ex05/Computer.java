...
public interface Computer {
    ...
    // 규칙을 추가하게 된다.
    //void touch();  // 추가하는 순간 기존 클래스들의 컴파일 오류 발생
    
    // 문제점
    // => 규칙 변경시, 그 규칙에 따라 만든 모든 클래스를 변경해야 한다.
    ...
    // 새 규칙을 새 인터페이스로 정의할 순 없지만,
    // 그렇게 하면 기존에 진행한 모든 프로젝트들과 호환 되지 않는 문제가 발생한다.
    
    // 기존 규칙을 변경하되, 기존 구현체에는 영향을 끼치고 싶지 않을 때
    // 바로 다음 문법을 사용하라!!!!!!!!!!!!!!!!!!
    // "디폴드 메서드(default method)" - JDK8 부터 추가한 문법이다.!!!!!!!!!!!!!!
    default void touch() {
        // 구현할 코드 있으면 작성하고 없으면 빈 채로 내버려 둔다.
    }
    
    // 위의 touch()라는 새 규칙을 추가하더라도
    // 기존의 작성한 FirstComputer, Secondcomputer, ThirdComputer 클래스 모두
    // 컴파일 오류가 발생하지 않는다.
    // 왜? 구현된 메서드이기 때문이다.
    
    // 가능한 일반 클래스의 메서드처럼 사용 하지 말아라!
    // 새 규칙을 추가하는 의미로 default 메서드를 사용해야지
    // 일반 클래스의 메서드를 만들 듯이 상속 해 줄 목적으로 default 메서드를 만들어서는 안된다.
}
