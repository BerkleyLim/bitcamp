...
public class Exam03 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 이 변수를 사용할 때는 클래스 로딩 후 static {} 블록을 실행하지 않는다.
        // 그러나 그 이후에 스태틱 멤버를 사용하는 최초의 순간에
        // 결국 static 블록은 실행 될 것이다.
        //Class clazz = Class.forName("step18.ex01.Exam03$A");
        
        // forName()을 호출하는 것 보다 안 좋은 이유는?
        // forName()의 파라미터는 문자열이다.
        // 즉 외부에서 문자여을 입력 받아 해당 클래스를 임의로 로딩할 수 있지만,
        // "class"라는 스태틱 변수를 사용하는 것은
        // 자바 소스 안에 명확히 해당 클래스를 지정해야 하기 때문에
        // 임의로 다른 클래스를 로딩하는 코드를 작성 할 수 없다.
        // 즉 다른 클래스를 로딩하고 싶으면 소스 코드를 변경해야 한다.
        // 하지만 이 방식으로 클래스를 로딩 시 static {} 블록을 실행 않음
        // 물론 스태틱 멤버를 사용 시 최초의 순간에도 static 블록이 실행될 것이기 때문에
