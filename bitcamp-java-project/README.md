...
##src 47 - PageController 인터페이스 대신 애노테이션을 사용하여 호출할 메서드를 지정하기
학습목표
...
작업내용
    - include, forward를 JSP 전용태그로 처리한다.
...
작업내용
    ...
    - 로그인 후 Referer 헤더 값을 참조하여 이전 페이지로 돌아간다.
...
## src27 - Serializable/deserialize 기법을 적용하여 객체 데이터를 관리하라!
학습목표
...
## src26 - Command 디자인패턴을 적용하여 명령어 단위로 클래스로 쪼개라!
학습목표
...
학습목표
  ...
  - 예외를 처리하는 방법을 익힌다.
작업내용
  - 사용자 입력에서 오류가 발생했을 때 적절한 조치를 취한 후에 계속 시행하도록 만든다.

## src24 - File I/O를 적용하여 데이터를 보관하라!
학습목표
  - File I/O 사용법을 익힌다.
작업내용
  - DAO에 파일 입출력을 적용하여 데이터 퍼시스턴스(persistance)를 관리한다.
...
- 작업내용
  ...
  - 추상 클래스와 추상 메서드를 적용한다.

## src21 - 컨트롤러에 인터페이스 적용
- 학습목표
  - 인터페이스의 정의와 활용 방법을 익힌다.
- 작업내용
  - App 클래스와 XxxxController 클래스 사이에 호츌 규칙을 인터페이스로 정의한다.
...
## src19 - 자바에서 제공하는 컬렉션 클래스를 적용하라. 또한 제네릭을 사용하라! (src18 포함)
- 학습목표
...
- 작업내용
  - DAO 클래스에서 객체 목록을 다루는 코드를 별도의 ArrayList 클래스로 분리한다.
...
명령> task/add 비트비트
작업명? UI 프로토타입 만들기
시작일? 2018-1-1 (시작일을 지정하지 않으면 팀 시작일 값을 사용한다.)
종료일? 2018-1-14 (종료일을 지정하지 않으면 팀 종료일을 사용한다.)
작업자? hong (작업자를 지정하지 않으면 null 값을 저장한다.)

명령> task/add 비트비트
작업명? UI 프로토타입 만들기
시작일? 2017-12-20 (시작일이 팀 시작일 보다 이전이면 팀 시작일을 값으로 설정한다.)
종료일? 2018-12-7 (종료일이 팀 종료일 보다 크면 팀 종료일을 값으로 사용한다.)
작업자? park (존재하지 않은 사용자 아이디일 경우 null 값으로 저장한다.)
...
작업의 상태를 '작업완료'(으로) 변경하였습니다.

명령> task/state 비트비트
상태를 변경할 작업의 번호? 1
'UI 명세서 작성' 작업의 상태: 작업중(1)
변경할 상태? (0:작업대기, 1:작업중, 9:작업완료) 5
올바르지 않은 값입니다. 이전 생태를 유지합니다!
...
## src16 - 캡슐화 적용
- 학습목표
  - 사용자 정의 데이터 타입에 대해 캡슐화를 적용하여 사용한다.
- 작업내용
  - domain 패키지에 있는 사용자 정의 데이터 타입 클래스에 갯터, 셋터를 적용한다.
...
## src13 - 날짜 값은 다루는 java.sql.Date 클래스 적용(리팩토링)
- 학습목표
  - java.sql.Date 클래스의 사용법을 익힌다.
- 작업내용
  - 팀, 회원, 게시물 정보 입력/변경과 회원 정보 입력/변경시 날짜 값을 Data 객체에 저장한다.
...
설명(aaaaaaa)? okokok
최대인원(5)? 10
시작일(2018-05-05)? 2018-04-10
종료일(2018-08-20)? 2018-8-8
변경하였습니다.
